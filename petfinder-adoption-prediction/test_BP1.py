...
train = pd.read_csv('./splitbp/test_split.csv', engine='python')
...
with tf.Session() as sess:
    ...
    for batch_idx in range(total_batches):
        ...
        aa = kappa(gt, result)
        kappa0 = kappa0 + aa
        final_result = np.append(result, final_result)

    # calculate average kappa
    kappa1 = kappa0/total_batches
    print(final_result.shape)
    print("Test accuracy:", kappa1)
    submission_BP = pd.DataFrame({'PetID': sub.PetID, 'AdoptionSpeed': [int(i) for i in final_result]})
    # submission_BP = pd.DataFrame({'PetID': sub.PetID, 'AdoptionSpeed': [int(i) for i in submission000]})
    submission_BP.head()

    submission_BP.to_csv('submission_BP.csv', index=False)
